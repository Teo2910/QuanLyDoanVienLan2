  ...
  return (
    <div className="h-full flex flex-col gap-6 overflow-hidden p-6">
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6 shrink-0 pt-2">
        <div>
          <h2 className="text-3xl font-bold text-white tracking-tight">Hoạt động sắp tới</h2>
          <p className="text-white/40 mt-1 text-[10px] uppercase tracking-widest">Lịch trình và sự kiện Đoàn thanh niên cơ sở</p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-4 w-full lg:w-auto items-stretch sm:items-center">
          <div className="relative flex-1 sm:w-64">
            ...
            <input 
              ...
              className="w-full bg-white/5 border border-white/10 rounded-full py-2 pl-12 pr-6 text-xs text-white placeholder:text-white/20 focus:outline-none focus:ring-1 focus:ring-accent/50 transition-all font-medium"
              value={searchTerm}
          ...
          {(isAdmin || isSecretary) && (
            <motion.button 
              ...
              className="group flex items-center justify-center gap-2 px-6 py-2.5 bg-accent border border-white/20 rounded-xl transition-all shadow-xl shadow-accent/10 hover:bg-white shrink-0"
            >
              <Plus size={16} className="text-slate-950 group-hover:rotate-90 transition-transform" />
              <span className="font-bold uppercase tracking-widest text-[9px] text-slate-950">Thêm hoạt động</span>
            </motion.button>
          )}
        </div>
      </div>
