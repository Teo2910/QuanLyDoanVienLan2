...
import { Bot, Send, User, ChevronRight, Database, Loader2 } from "lucide-react";

export const AIAssistant = () => {
  const [query, setQuery] = useState("");
  const [chatHistory, setChatHistory] = useState<{ role: "user" | "ai", text: string }[]>([
    { role: "ai", text: "Xin chào! Tôi là Trợ lý AI. Tôi đã được kết nối với cơ sở dữ liệu của hệ thống. Bạn có thể hỏi tôi bất kỳ thông tin nào về đoàn viên, chi đoàn hoặc các hoạt động." }
  ]);
...
`;
      ...
      const response = await ai.models.generateContent({
        ...
        contents: [
          ...
          { role: "model", parts: [{ text: "Đã hiểu, hãy đặt câu hỏi." }] },
          ...chatHistory.filter(m => m.role === "user").map(m => ({ role: "user", parts: [{ text: m.text }] })),
          { role: "user", parts: [{ text: userText }] }
        ]
      });

      setChatHistory(prev => [...prev, { role: "ai", text: response.text || "Tôi không có câu trả lời cho vấn đề này." }]);
    } catch (err: any) {
      console.error(err);
      setChatHistory(prev => [...prev, { role: "ai", text: "Xin lỗi, đã có lỗi xảy ra khi kết nối với mô hình AI." }]);
    } finally {
      setIsLoading(false);
    }
  };
